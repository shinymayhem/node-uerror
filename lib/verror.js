...
/*
 * Like VError, but inherits name from cause
 */
function UError(options)
{
	var args, obj, causedBy, ctor, tailmsg;
	 ...
	 * without "new".
	 */
	if (!(this instanceof UError)) {
		args = Array.prototype.slice.call(arguments, 0);
		obj = Object.create(UError.prototype);
		UError.apply(obj, arguments);
		return (obj);
	}
	 ...
	 * produce bogus output.
	 *
	 * However, sometimes the bug is only in the code calling UError, and a
	 * programmer might prefer to have the error message contain "null" or
	 * "undefined" rather than have the bug in the error path crash the
	 * program (making the first bug harder to identify).  For that reason,
	 * by default UError converts "null" or "undefined" arguments to their
	 * string representations and passes those to extsprintf.  Programmers
	 * desiring the strict behavior can use the SError class or pass the
	 * "strict" option to the UError constructor.
	 */
	if (!options || !options.strict) {
		args = args.map(function (a) {
			return (a === null ? 'null' :
...
mod_util.inherits(UError, Error);
UError.prototype.name = 'UError';

UError.prototype.toString = function ve_toString()
{
	var str = (this.hasOwnProperty('name') && this.name ||
		this.constructor.name || this.constructor.prototype.name);
	if (this.message)
...
UError.prototype.cause = function ve_cause()
{
	return (this.jse_cause);
};
